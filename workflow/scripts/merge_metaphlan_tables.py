...
def merge(aaastrIn, ostm, gtdb, col):
    """
    ...
    for f in aaastrIn:
        ...
        iIn = pd.read_csv(f, sep='\t', skiprows=len(headers), names=names, usecols=[0,2,4] if not gtdb else range(2), index_col=0)
        profiles_list.append(pd.Series(data=iIn[col], index=iIn.index,
...
argp.add_argument('-c', default="relative_abundance", help="Name of column to be merged")
...
def main( ):
    ...
    merge(args.aistms, open(args.o, 'w') if args.o else sys.stdout, args.gtdb_profiles, args.c)
